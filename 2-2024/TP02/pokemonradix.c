...
char* ListaparaString(Lista lista) {
    char *resp = (char*)malloc(500 * sizeof(char));
    for(int i = 0; i < lista.n - 1; i++){
            strcat(resp,"'");
            strcat(resp,lista.array[i]);
            strcat(resp,"'");
          if(lista.n > 1 && i < lista.n - 2){
            strcat(resp,",");
          }
       }
    return resp;
...
int getMaior(Pokemon **pokemons, int n) {
    int max = 0;
    char abilitiesStr[500];
    
    for (int i = 0; i < n; i++) {
       strcpy(abilitiesStr,ListaparaString(pokemons[i]->abilities));
       movimentacoes++;
        int len = strlen(abilitiesStr);
        if (len > max) {
            max = len;
        }
    }
...
void countingSort(Pokemon **p, int n, int exp) {
    Pokemon **output = (Pokemon **)malloc((n + 1) * sizeof(Pokemon *));
    int count[256] = {0};  // Para caracteres ASCII

    char abilitiesStr[500];

    // Contar a frequência dos caracteres nas posições de exp
    for (int i = 0; i < n; i++) {
        strcpy(abilitiesStr,ListaparaString(p[i]->abilities));
        int index = (exp < strlen(abilitiesStr)) ? (unsigned char)abilitiesStr[exp] : 0;
        count[index]++;
        comparacoes++;
        movimentacoes++;
    }
    ...
    // Construir array de saída, respeitando critério de desempate por nome
    for (int i = n - 1; i >= 0; i--) {
        strcpy(abilitiesStr,ListaparaString(p[i]->abilities));
        int index = (exp < strlen(abilitiesStr)) ? (unsigned char)abilitiesStr[exp] : 0;
...
void radixsort(Pokemon **p, int n) {
    int max = getMaior(p, n);  // Obtém o tamanho máximo de abilities como string

    for (int exp = max - 1; exp >= 0; exp--) {
        countingSort(p, n, exp);
    }
}

int main()
{
    FILE *matricula = fopen("859230_radixsort.txt", "w");
    ...
    inicio = clock();
    selecao(p, tamvet);
    radixsort(p, tamvet);//ordenaçao
    for(int i = 0; i < tamvet; i++){
        imprimir(*p[i]);
        //printf("1");
        //printf("%s\n",ListaparaString(p[i]->abilities));
    }
    final = clock();
