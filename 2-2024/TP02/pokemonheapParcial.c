...
int comparePokemon(Pokemon *p1, Pokemon *p2) {
    if (p1->height > p2->height) {
        ...
        comparacoes++;
        return strcmp(p1->name, p2->name);  // Desempate por nome
    }
}

int getMaiorFilho(int i, int tamHeap, Pokemon *array[]) {
    int filho;
...
void construir(int tamHeap, Pokemon *array[]) {
...
void reconstruir(int tamHeap, Pokemon *array[]) {
    int i = 1;
    while (i <= (tamHeap / 2)) {
        ...
        if (comparePokemon(array[i], array[filho]) < 0) {
            ...
            i = tamHeap; // Sair do loop
        }
    }
}

void heapsort(int n, Pokemon *array[]) {
    int k = 10;
    ...
    for(int i = k + 1; i <=n; i++){
        if (comparePokemon(tmp[i], tmp[1]) < 0) {
            swap(tmp[i], tmp[1]);
            reconstruir(k, tmp);
            movimentacoes+=3;
        }
...
int main()
{
    FILE *matricula = fopen("859230_heap-parcial.txt", "w");
