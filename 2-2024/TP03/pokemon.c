 ...
 * @param capacidade Capacidade inicial da lista.
 */
void startP(ListaPokemon *l, int capacidade)
{
    l->n = 0;
    l->tamanho = capacidade;
 ...
 * @param x char* string a ser inserida.
 */
void inserirInicioP(ListaPokemon *l, Pokemon *x)
{
    int i;
 ...
 * @param x char* string a ser inserida.
 */
void inserirFimP(ListaPokemon *l, Pokemon *x)
{
    // Validar inserção
    if (l->n >= l->tamanho)
    {
 ...
 * @param pos Posicao de inserção.
 */
void inserirP(ListaPokemon *l, Pokemon *x, int pos)
{
    int i;
 ...
 * @return resp char* string a ser removida.
 */
Pokemon removerInicioP(ListaPokemon *l)
{
    int i;
    Pokemon resp;
 ...
 * @return resp char* string a ser removida.
 */
Pokemon removerFimP(ListaPokemon *l)
{
    // Validar remoção
    if (l->n == 0)
    {
 ...
 * @return resp char* string a ser removida.
 */
Pokemon removerP(ListaPokemon *l, int pos)
{
    int i;
    Pokemon resp;
...
int main()
{
    ...
    while (fim == 0)
    {
        ...
        else
        {
            ...
            inserirFimP( &listaP, p[toInt(entrada)-1]);
        }
    }
    scanf(" %d", &fim);
    Pokemon tmp;
    for(int i = 0; i < fim; i++){
        ...
        if(strcmp(entrada, "II") == 0){
            scanf(" %s", entrada);
            inserirInicioP(&listaP, p[toInt(entrada)-1]);
            ...
            scanf(" %d", &pos);
            scanf(" %s", entrada);
            inserirP(&listaP, p[toInt(entrada)-1], pos);
        }else if(strcmp(entrada, "RI") == 0){
            tmp = removerInicioP(&listaP);
            printf("(R) %s\n", tmp.name);
        }else if(strcmp(entrada, "RF") == 0){
            tmp = removerFimP(&listaP);
            printf("(R) %s\n", tmp.name);
        }else if(strcmp(entrada, "R*") == 0){
            scanf(" %s", entrada);
            tmp = removerP(&listaP, toInt(entrada));
            printf("(R) %s\n", tmp.name);
        }
    }
    mostrarP(&listaP);
