...
int main(int argc, char const *argv[])
{
    ...
    fim = 0;
        ...
        while(fim == 0) {
            scanf(" %[^\r\n]", entrada);//pega a linha inteira incluindo os espaços
            if(strcmp(entrada, "FIM") == 0){//se a entrada for fim, sai do loop e termina o programa
                fim = 1;
            }else{
                //printf("oii\n");
                esq = 0;
                ...
                while(esq <= dir){
                    meio = (esq+dir)/2;
                    //printf("%s | %s\n", P[meio].nome, entrada);
                    if(strcmp(entrada, P[meio].nome) == 0){
                        printf("SIM\n");
                        esq = dir;
                        resp = 1;
                    }else if(strcmp(entrada, P[meio].nome) < 0){
                        dir = meio -1;
                    }else{
                        esq = meio +1;
                    }
